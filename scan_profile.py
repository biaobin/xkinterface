...
def gen_gen4in(Lbuncht, Ipeak, folderName='.',slsc="ON",llsc="ON"):
...
for Ipeak in Ipeakl:
    for Lbuncht in Lbunchtl:
        ...
        if profile=='parabolic':
            sigz = Lbuncht/6*c
            tmpQ = 13.3422*Ipeak*sigz*1e-9
        elif profile=='flattop':
            tmpQ=Ipeak*0.9*Lbuncht
        print(f"Q={tmpQ*1e9:.2f} nC")  

        #mkdir a new folder
        folderName = f"Ipeak{Ipeak:.0f}A_Lbuncht{Lbuncht*1e12:.2f}ps"
        os.makedirs(folderName, exist_ok=True)        
        os.chdir(folderName)
        
        #for impz
        os.makedirs("01_impz", exist_ok=True)
        Q = gen_impzin(profile=profile,Lbuncht=Lbuncht,Ipeak=Ipeak, Np=Np, folderName='01_impz')
        
        #for gen4
        os.makedirs("02_gen4", exist_ok=True)
        # shutil.copy2(f"{rootdir}/runFile/gen4.lat", "02_gen4")        
        gen_gen4in(Lbuncht,Ipeak,folderName="02_gen4",slsc=slsc,llsc=llsc)
        
        # Now, for one & submit.sh
        gen_one(Q,lamdas=lamda0,path=".",)        
