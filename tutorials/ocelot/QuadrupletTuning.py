...
if __name__ == "__main__":
    ...
    if True:
        ...
        combi = np.array([[grad0, grad1, v2, 0] for v2 in var2])
        ...
        if not Q2_scanned:
                ...
                if 1:
                    ...
                    while nTrials<maxTrials:
                        ...
                        if len(res)>=2:
                            ...
                            func = interp1d(data[:,8]-data[:,9], data[:,2], fill_value = 'extrapolate') # rms size match
                            #func = interp1d(data[:,10]-data[:,11], data[:,1], fill_value = 'extrapolate') # beta match
                            tmp = func([0])[0]                            
                
                        else:
                            if result[0][4]*np.sign(grad1)>0:
                                tmp = grad2*0.75
                            else:
                                tmp = grad2*1.25
            ...
            fig, ax = plt.subplots()
            ax.plot(data[:,2], data[:,8], '-*')
            ax.plot(data[:,2], data[:,9], '-o')
            ax.grid()
        ...
        combi = np.array([[grad0, grad1, grad2, v3] for v3 in var3])
        ...
        if not Q3_scanned:
                ...
                if 0:
                    ...
                    while 1:
                        if len(res)>=2:
                            ...
                            select = (data[:,4]<1e4); data = data[select]
                            data = data[np.abs(data[:,3]).argsort()[::]]
    
                            #func = interp1d(data[:,3], data[:,2], fill_value = 'extrapolate') # rms size match
                            func = interp1d(data[:,8]-data[:,9], data[:,3], fill_value = 'extrapolate') # rms size match
            ...
            fig, ax = plt.subplots()
            ax.plot(data[:,3], data[:,8], '-*')
            ax.plot(data[:,3], data[:,9], '-o')
            ax.grid()
            ax.set_xlabel(r'$G$ (T/m)')
            ax.set_ylabel(r'rms size (mm)')
            fig.savefig('rms-size-vs-grad3@%.2fT_m.eps' % grad1)
    ...
    if Zfinal > 0:
        ...
        def func_wrapper(x):
            r = ObjOcelot(x, zquad, Distribution = Distribution, Zstart = z0,
                        ...
                        Distribution1 = 'ast.%04d.%03d' % (Zfinal*100, Run0))
            return [r]
